...
public:
    explicit Test_component(const Cecilion::Entity_ref &ref) : Cecilion::I_Component(
        std::forward<const Cecilion::Entity_ref>(ref)) {
    }
    ...
    App() {
            {
                ...
                // FIX!!!!
                //scene.get_component<Cecilion::Transform>().operator->().entity().operator->().m_entity_ID;
                try {
