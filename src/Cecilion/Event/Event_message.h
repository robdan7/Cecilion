...
namespace Cecilion {
    ...
    const unsigned int KEYBOARD_EVENT = __COUNTER__;
    ...
    struct Keyboard_Event : public Event_message {
        const int key, scancode, action, mods;
        Keyboard_Event(int key, int scancode, int action, int mods) : Event_message(KEYBOARD_EVENT), key(key), scancode(scancode), action(action), mods(mods) {}
    };
}
