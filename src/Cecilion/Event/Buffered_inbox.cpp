...
#include <Core/Log.h>
namespace Cecilion {
    void Buffered_inbox::unsubscribe(int message_ID) {
    ...
    void Buffered_inbox::subscribe_to(int message_ID, I_Event_inbox::Event_callback callback) {
    ...
    void Buffered_inbox::execute_inbox() {
        ...
        for (Buffered_event event : *this->p_message_stack) {
            event.callback(this->p_event_parent, event.event.get());
        }
