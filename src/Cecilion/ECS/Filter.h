...
namespace Cecilion {
    ...
    public:
        ...
        Filter(Cecilion::Component_storage<ECS_ENTITY_SIZE ,Component>*... com) {
            this->containers = {com...};
        }
        ...
        auto get_smallest_sparse () {
            ...
            return std::min({
                                    static_cast<const Cecilion::Sparse_set<ECS_ENTITY_SIZE>*>(
                                            std::get<Cecilion::Component_storage<ECS_ENTITY_SIZE,Component>*>(this->containers)
                                    )...
                            }
        ...
        void for_each_sparse(Function func) {
            ...
            for (int page = 0; page < page_size; page++) {
                ECS_ENTITY_SIZE ID = 0;
                for (int offset = 0; offset < smallest_container->index_per_page; offset++) {
                    ID = smallest_container->ID(page, offset);
                    if (auto all = (std::get<Cecilion::Component_storage<ECS_ENTITY_SIZE, Component> *>(containers)->has_ID(
                                ID) &&...); all) {
                        if constexpr (std::is_invocable_v<Function, decltype(std::get<Cecilion::Component_storage<ECS_ENTITY_SIZE, Component> *>(
                                containers)->get(ID))...>) {
//                        my_test_function(std::get<Cecilion::Component_storage<Entity::Entity_ID,Comps>*>(containers)->get(ID)...);
                            func(std::get<Cecilion::Component_storage<ECS_ENTITY_SIZE, Component> *>(containers)->get(ID)...);
                        } else {
        ...
        void for_each_compact(Function func) {
            ...
            smallest_container->for_each_packed([this, func](auto ID){
                if (auto all = (std::get<Cecilion::Component_storage<ECS_ENTITY_SIZE, Component> *>(containers)->has_ID(
                            ID) &&...); all) {
                    if constexpr (std::is_invocable_v<Function, decltype(std::get<Cecilion::Component_storage<ECS_ENTITY_SIZE, Component> *>(containers)->get(ID))...>) {
//                        my_test_function(std::get<Cecilion::Component_storage<Entity::Entity_ID,Comps>*>(containers)->get(ID)...);
                        func(std::get<Cecilion::Component_storage<ECS_ENTITY_SIZE, Component> *>(containers)->get(ID)...);
                    } else {
                        CORE_ASSERT(false, "Error: ECS: Component callback function does not contain the right types");
        ...
        std::tuple<Cecilion::Component_storage<ECS_ENTITY_SIZE, Component>*...> containers;
    };
}